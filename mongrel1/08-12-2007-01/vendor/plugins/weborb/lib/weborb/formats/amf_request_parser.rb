...
require 'weborb/io/binary_reader_fast'
...
class AmfRequestParser
  ...
  def read_message( input, isFast )
    reader = (isFast) ? BinaryReaderFast.new(input) : BinaryReader.new(input)
    ...
    formatter = version == 3 ? AmfV3Formatter.new(isFast) : AmfV0Formatter.new(isFast)
