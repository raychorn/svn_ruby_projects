=begin
THIS HAS BEEN MODIFIED FOR MOLTEN!!!!
    ...
    class SalesforceAdapter < AbstractAdapter
      ...
      def update(sql, name = nil) #:nodoc:
        #log(sql, name) {
          ...
          # modified - doesn't work for statements like:
          # UPDATE note SET parent_id = NULL WHERE ((parent_id = 'a16300000008RclAAE'))
          id = if (sql =~ /\s+\(\(/)
                 sql.match(/WHERE\s+\(\(\w+_id\s*=\s*'(\w+)'\)\)/mi)[1]
               else
                 sql.match(/WHERE\s+id\s*=\s*'(\w+)'/mi)[1]
               end
