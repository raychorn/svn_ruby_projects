...
module  AuthenticationSystem
  ...
  def current_contact
    id = if params[:cache_key] and params[:cache_key] == AppConstants::CACHE_KEY and params[:cached_contact_id]
            params[:cached_contact_id]
         else
           cookies['contact_id'] || (!response.cookies.nil? ? response.cookies['contact_id'] : nil) || (!request.cookies.nil? ? request.cookies['contact_id'] : nil)
         end
