...
class SalesforceWalker < ActiveRecord::Schema
  ...
  def self.expire_deleted(options = {})
    ...
    classes_hash.each do |salesforce_class, local_class|
      local_record_paginator = build_paginator(local_class,options) 
      
      # try to find the record in Salesforce...if it doesn't exist, delete the local record.
      local_record_paginator.each do |local_record|
  ...
  # returns a Paginator...if a the +time+ option is specified, adds conditions to limit
  # to the time range. 
  # TODO: Do we need to covert the time?
  def self.build_paginator(klass,options)
    ...
    if time_range = options[:time]
                                ...
                                klass.find(:all, :page_size => PAGE_SIZE, :conditions => conditions)
    else
      klass.find(:all, :page_size => PAGE_SIZE)
    end
  end
end