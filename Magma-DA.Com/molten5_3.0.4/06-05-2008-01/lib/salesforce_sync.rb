...
class SalesforceSync < ActiveRecord::Schema
  ...
  def self.update_schema
    @@logger.info("[#{Time.now}] Updating Schema...")
    ...
    @@logger.info("[#{Time.now}] Finished Updating Schema.")
  ...
  def self.update_records(*sf_classes)
    ...
    if (SfmoltenPost.check_downtime_interval)
        @@logger.info("[#{Time.now}] MOLTEN downtime scheduled. Exiting ...")
#        raise "MOLTEN downtime scheduled. Exiting ..."
    ...
    @@logger.info("[#{Time.now}] Updating Records:")
    ...
    @@logger.info("[#{Time.now}] Finished Updating Records.")
  ...
  def self.delete_records(salesforce_class,local_class,deleted_ids)
    deleted_ids.each do |id|
      begin
        ...
        @@logger.info("Deleted #{salesforce_class} with id #{id}")
      rescue Exception => e
        @@logger.info("Could not delete #{salesforce_class} with id #{id}")
        @@logger.info e
      end
    end
  end
  ...
  def self.do_delete_records(salesforce_class, local_class, updates_from, updates_to)
    ...
    begin
      ...
      if $!.message =~ /TypeError: Symbol as array index/ or $!.message =~ /NoMethodError/
        @@logger.info("No #{salesforce_class} records to delete for range #{updates_from} - #{updates_to}") 
      elsif $!.message =~ /not replicable/
        @@logger.info("This class isn't replicable [#{salesforce_class}]")
      else
        @@logger.info("Objects of class [#{salesforce_class}] must be a custom object")
        @@logger.info($!.message)
        begin
          @@logger.info("Attempting to find deleted custom objects of class [#{salesforce_class}]")
    ...
    @@logger.info("An error occured deleting records")
    @@logger.info($!.message)
    @@logger.info($!.backtrace)
  end #do_delete_records
  
  def self.sync_records(salesforce_class, local_class)
    @@logger.info("Syncing #{salesforce_class.to_s} records with #{local_class.to_s} records")
    ...
    begin
      ...
      @@logger.info("Update range set to #{updates_from} - #{updates_to}")
      ...
      updated_sf_record_ids = if salesforce_class == SelfServiceUser
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    @@logger.info("Found [#{ids.size}] records of type [#{salesforce_class.to_s}] that need to be updated.")
                                    ids
                                 end
                                end
                              end       
    rescue => e  
      @@logger.error("Error Occured During Sync: #{e}")       
    ...
    updated_sf_record_ids.each do |updated_id|  
      ...
      if local_record and sf_record
        ...
        @@logger.error("Warning - couldn't find the Salesforce Record to meet the conditions:")
  ...
  def self.insert_record(sf_cols,sf_record,salesforce_class,local_class)
    @@logger.info("Inserting new record for #{local_class.to_s} / #{sf_record.id}")
    ...
    begin
      if local_record.save
        ...
        @@logger.info("[Successfull Insert] Created #{local_class} / #{local_record.id} / #{local_record.last_modified_date}")
        ...
        @@logger.info("[Failure Insert] #{local_class} / #{sf_record.id}: #{local_record.errors.full_messages.join(',')}")
      end
 ...
 #    self.send_error_email(exception)
  ...
  def self.update_record(sf_record,local_record,salesforce_class,local_class)
    @@logger.info("Found a local record...updating #{local_class.to_s} / #{local_record.id}")
    sf_cols = salesforce_class.column_names
    ...
    @@logger.info("Updating #{local_class} / #{local_record.id}")  
    begin
    if local_record.update_attributes(updated_attributes)
      @@logger.info("[Successfull Update] Updated #{local_class.to_s} / #{local_record.id} / #{sf_record.system_modstamp}")
    else
      @@logger.info("[Failure Update] Occurred updating #{local_class.to_s} / #{local_record.id}: #{local_record.errors.full_messages.join(',')}")
    end
    rescue => e
  ...
  def self.sync_schema(salesforce_class,local_class)
    ...
    begin
      ...
      if $!.message =~ /doesn't exist/
        @@logger.info("Ignoring syncing of schema for #{local_class} as local table doesn't exist.")
        return
      else
        raise
      end
    ...
    sf_cols.each do |acol|
        ...
        if (colexist.to_s()=="False")
          @@logger.info("Inserting column..... #{colname}")
  ...
  def self.build_updated_attributes(sf_cols,sf_record,salesforce_class,local_class,local_record = nil)
    ...
    @@logger.info("[#{salesforce_class.to_s}/#{sf_record.id}] Updated Attributes:")
