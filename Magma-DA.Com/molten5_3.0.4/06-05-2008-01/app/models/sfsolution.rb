...
class Sfsolution < ActiveRecord::Base
  ...
  # - are published
  # - in the user's account list if they are account-specific
  def self.contact_viewing_restrictions(contact)
    ["status = '#{STATUSES[:published]}' AND (portal_account_name_id__c IS NULL OR portal_account_name_id__c IN (?))",contact.account_list.map(&:id)]
  end
  ...
  def self.find_popular_in_range(contact,options = {})
    ...
    results.reject! { |r| r.first.nil? or (!r.first.portal_account_name_id__c.nil? and !contact.account_list.map(&:id).include?(r.first.portal_account_name_id__c))}
  ...
  def self.new_solutions_in_range(contact,options = {})
    ...
    results.reject! { |r| !r.viewable? or (!r.portal_account_name_id__c.nil? and !contact.account_list.map(&:id).include?(r.portal_account_name_id__c)) }
