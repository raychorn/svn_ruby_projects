...
class ListService
			...
			args.list_subjects.each do |list_subject|
				if (list_subject.data_filters) then
					list_subject.data_filters.each do |data_filter|
					  puts "parsing data_filter = #{data_filter.inspect}"
      				  ...
      				  if (real_property.is_enum) then
      				    sql_where_by_prop_id[data_filter.property.id] << data_filter.generateSQL(real_property.ar_class.tableize + ".id")
      				  else
    ...
    if (args.class == ListAnalysis) then
      ...
      #     ]
        ...
        # load up some common stuff
        report_subject = ReportSubject.find(property.report_subject_id)

        # generate link template for details
        if (property.name.include?(' Name')) then
  	      subjectDetails = SubjectDetails.new
  	      subjectDetails.className = report_subject.table_name.classify
          replacementMaps = [{
            :columnName => "#{report_subject.table_name}_id",
            :analysisOptionPropertyDescriptor => ['id']
          }]
  	      column_meta_data[property.name][:links] << {
  	        :name => 'View Details',
  	        :currentAnalysisOptions => subjectDetails,
  	        :replacementMaps => replacementMaps
  	      }
        end
        
        # generate link template for enumerable properties
        if (property.is_enum) then
  	      listAnalysisTemplate = nil
  	      replacementMaps = []
  	      # copy the ListAnalysis options
  	      listAnalysisTemplate = args.deep_copy
  	      # deal with sort_columns. blank them out for now. perhaps something smarter later...
  	      listAnalysisTemplate.sort_columns = [];
  	      # find the report subject with this property and add a data filter specifying 'this' value
  	      
  	      listAnalysisTemplate.list_subjects.each_with_index do |list_subject, index|
  	        if (property.report_subject_id == list_subject.subject.id) then
  	          pkDataFilter = DataFilter.new
  	          pkDataFilter.property = property.deep_copy
  	          pkDataFilter.property_operator = PropertyTypeOperator.find(case property.property_type_id
                when DSSConstants::PropertyTypeID
                  DSSConstants::PropertyType_ID_Operator_IsInSet
                when DSSConstants::PropertyTypeString
                  DSSConstants::PropertyType_String_Operator_Is
                when DSSConstants::PropertyTypeNumber
                  DSSConstants::PropertyType_Number_Operator_Equals
                when DSSConstants::PropertyTypeIP
                  DSSConstants::PropertyType_IP_Operator_Is
                when DSSConstants::PropertyTypeBoolean
                  DSSConstants::PropertyType_Boolean_Operator_Is
    	        end)
  	          pkDataFilter.value = nil
  	          list_subject.data_filters << pkDataFilter
  	          replacementMaps << {
  	            :columnName => property.name,
  	            :analysisOptionPropertyDescriptor => ['list_subjects',index,'data_filters',(list_subject.data_filters.length - 1),'value']
  	          }
  	        end
  	      end
  	      column_meta_data[property.name][:links] << {
