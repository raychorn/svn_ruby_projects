...
class ClassUtil
  ...
  def ClassUtil.load_class( class_name, source_path=nil )
    if source_path.nil? || source_path == class_name
      if class_name.index('.')
      require_string = class_name.gsub(/\./, "/")
      begin
        require_dependency(require_string + ".rb")
      rescue Exception => exception
        Log.info("unable to load source file, " + require_string + ", hopefully Rails has already loaded it: #{exception}")
      end
      ...
      begin
        require_dependency require_string
      rescue Exception => exception
        Log.info("unable to load source file, " + require_string + ", hopefully Rails has already loaded it: #{exception}")
      end
      Object.const_get class_name
