...
FAC3 - Fixlet Automated Creation 3! (Turned 3 with new BDE Schema)
...
=item decodeTextField($fields)

    Takes the fields string and converts it to an array

=cut

sub decodeTextFields {
...
sub runInsert_Object {
    my $this = shift;
    
    
}

sub runApprove_Object {
    my $this = shift;
}
...
sub insertTextfields {
	...
	my $isFixlet 	= $this->getIsFixlet();
	my $version 	= $this->getVersion();
        my $fieldData   = encodeTextFields($this->getFields());
        
        my $sql = qq{
                INSERT INTO TextFields
                        (Sitename, ID, IsFixlet, Version, Fields)
                VALUES
                        ('$siteName', $id, $isFixlet, $version, 0x$fieldData)
        };                
        my $sth = $dbh->prepare($sql);
        dbLog("Inserting $siteName, $id into the database", $this->getLogFile());
        $sth->execute();
        $sth->finish();
}
