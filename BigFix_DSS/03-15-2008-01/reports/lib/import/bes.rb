  ...
  module BES
    ...
    class Database
      ...
      def get_windows_apps_cursor()
        get_analysis_results_cursor(audit_site_id(), 34, 1)
#        get_universal_analysis_cursor('Installed Applications')
    ...
    class Importer
      ...
      def paged_import(fetchFunction, transformFunction, insertFunction, options={}, page_size=1000)
      ...
      def import_computer_groups(options={})
        ComputerGroup.find(:all).each do |cg|          
          bes_computer_groups = cg.relevance ? cg.relevance.split : []
        ...
        @qna.query("(id of it, name of it) of bes computer groups").each do |dsg|
           id, name = dsg.split(', ')
           puts("Insert #{name} (#{id})")
           @dss_database.execute(@dss_database.sql_insert_datasource_computer_group(id, name)) \
             unless @dss_database.has_datasource_computer_group(id, name)
        end
      end
