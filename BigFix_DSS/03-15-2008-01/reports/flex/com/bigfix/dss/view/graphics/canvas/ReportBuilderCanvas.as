package com.bigfix.dss.view.graphics.canvas {
	...
	public class ReportBuilderCanvas extends UIComponent {
		...
		[Event(name="beginAddingElementsToReport", type="com.bigfix.dss.view.graphics.canvas.events.BeginAddingElementsToReportEvent")]
		[Event(name="endAddingElementsToReport", type="com.bigfix.dss.view.graphics.canvas.events.EndAddingElementsToReportEvent")]
		...
		private var _widgetMenu:WidgetMenu;

		private var _isWidgetMenuCalledFor:Boolean = false;
		
		private var _isWidgetMenuUsed:Boolean = false;
		...
		public function ReportBuilderCanvas() {
			...
			this.addEventListener(DeleteReportElementEvent.TYPE_DELETE_REPORT_ELEMENT, onDeleteReportElement);
			this.addEventListener(MovedReportElementEvent.TYPE_MOVED_REPORT_ELEMENT, onMovedReportElement);
		...
		public function deselectAllButThisChildById(id:String):void {
		...
		public function init():void {
			...
			try {
				...
				while ((child = this.getChildAt(i)) != null) {
					if ( (child is RubberBandBox) || (child is WidgetMenu) ) {
						i++;
					} else {
		...
		private function populateSpecificElement(bVO:ReportBuilderVO):void {
			this.dispatchEvent(new AddElementToReportEvent(AddElementToReportEvent.TYPE_ADD_ELEMENT_TO_REPORT, bVO.shape_type, bVO.x, bVO.y, bVO.width, bVO.height, bVO));
		}
		
		public function populateFrom(vo:WebOrbResultVO):void {
			var data:Array = vo.data as Array;
			if (data != null) {
				var i:int;
				var bVO:ReportBuilderVO;
				this.dispatchEvent(new BeginAddingElementsToReportEvent(BeginAddingElementsToReportEvent.TYPE_BEGIN_ADDING_ELEMENTS_TO_REPORT));
				for (i = 0; i < data.length; i++) {
					if (data[i] is ReportBuilderVO) {
						bVO = ReportBuilderVO(data[i]);
					} else if (data[i] is Array) {
						var datum:Array = data[i] as Array;
						bVO = ReportBuilderVO(datum[0]);
						bVO.scaleImage = true;
					}
					this.populateSpecificElement(bVO);
				}
				this.dispatchEvent(new EndAddingElementsToReportEvent(EndAddingElementsToReportEvent.TYPE_END_ADDING_ELEMENTS_TO_REPORT));
			}
		}
		
		public function deleteSelectedElement(widget:* = null):void {
			if (widget != null) {
		...
		private function addWidgetMenu():void {
			this._widgetMenu = new WidgetMenu();
			this._widgetMenu.visible = false;
			this.addChild(this._widgetMenu);
		}

		public function showWidgetMenu(isShown:Boolean, pt:Point, width:Number):void {
			if (isShown) {
				this._widgetMenu.x = pt.x;
			    this._widgetMenu.y = pt.y - 24;
			    this._widgetMenu.width = ((width < 120) ? 120 : width);
			    this._widgetMenu.height = 40;
			    this._widgetMenu.visible = false; // isShown;
				this.moveToFront(this._widgetMenu);
			}
		}
		...
		private function showWidgetMenuForWidget(widget:DisplayObjectContainer):void {
			if (widget != null) {
				var pt:Point = new Point(widget.x, widget.y);
				this.showWidgetMenu(true, pt, widget.width);
			}
		}
		
		private function onShowWidgetMenu(event:ShowWidgetMenuEvent):void {
			var widget:DisplayObjectContainer = DisplayObjectContainer(event.widget);
			this.showWidgetMenuForWidget(widget);
			this._isWidgetMenuCalledFor = true;
		}
		
		private function onIgnoreMouseUp(event:IgnoreMouseUpEvent):void {
		}
		
		private function onImageSelected(event:ImageSelectedEvent):void {
			var widget:WidgetCanvas = WidgetCanvas(this._currentSprite);
			if (widget != null) {
				widget.image = event.source;
				var imageSize:Array = event.size;
				try { 
					widget.width = imageSize[0];
					widget.height = imageSize[1];
				} catch (err:Error) { }
			}
		}
		
		private function onEditReportElement(event:EditReportElementEvent):void {
			this._isWidgetMenuUsed = true;
			this._currentSprite = event.widget;
			var shapeType:String = this._currentSprite.shapeType;
			var vo:ReportBuilderVO = this._currentSprite.serializeForRuby;
			if (vo.isImage) {
    			var win2:ImageEditor = this.popUpImageEditor(this._currentSprite.width, this._currentSprite.height);
	            win2.addEventListener(ImageSelectedEvent.TYPE_IMAGE_SELECTED, onImageSelected);
			} else if (!vo.isWidget) {
    			var win:RichTextEditor = this.popUpRichTextEditor(this._currentSprite.width, this._currentSprite.height);
    			win.sprite = this._currentSprite;
			}
		}

		private function onResizeReportElement(event:ResizeReportElementEvent):void {
			AlertPopUp.info("Selected Widget: " + this._currentSprite.id, "onResizeReportElement");
		}
	    ...
	    public function addWidget(widget:DisplayObjectContainer):DisplayObjectContainer {
            ...
            widget.addEventListener(ShowWidgetMenuEvent.TYPE_SHOW_WIDGET_MENU, onShowWidgetMenu);
            this.addEventListener(IgnoreMouseUpEvent.TYPE_IGNORE_MOUSE_UP, onIgnoreMouseUp);
            this.addEventListener(EditReportElementEvent.TYPE_EDIT_REPORT_ELEMENT, onEditReportElement);
            this.addEventListener(ResizeReportElementEvent.TYPE_RESIZE_REPORT_ELEMENT, onResizeReportElement);
		...
		private function onFileUploadCompleted(event:FileUploadCompletedEvent):void {
			...
			} else if (this._isWidgetMenuUsed == false) { // remove the child when the user hits the dismiss button because there was no image to upload.
				if (this._currentSprite is WidgetCanvas) {
					widget = WidgetCanvas(this._currentSprite);
					child = ImageWidget(widget.getChildAt(0));
	    ...
	    private function popUpImageEditor(width:Number = 500, height:Number = 250, initialImageCreation:Boolean = false):ImageEditor {
	    	var minWidth:Number = 550;
	    	var minHeight:Number = ((initialImageCreation) ? 150 : 300);
			...
			window.initialImageCreation = initialImageCreation;
			window.busy = this.busy;
			window.styleName = "ImageEditor";
	    ...
	    private function onMovedReportElement(event:MovedReportElementEvent):void {
	    	this.showWidgetMenuForWidget(this._currentSprite);
	    }
			...
			if (event.data == null) {
		    	switch (event.elementType) {
		    		...
		    		case ReportBuilderCanvas.const_IMAGE_ELEMENTS:
		    			var win2:ImageEditor = this.popUpImageEditor(event.width, event.height, true);
		    		break;
		    	}
			}
	    }
		...
		private function onMouseUp(evt:MouseEvent):void {
			...
			if (selectedSprite != null) {
				if ( (this._widgetMenu.visible) && (this._isWidgetMenuCalledFor == false) && (this._isIgnoringMouseUp == false) ) {
					this._widgetMenu.visible = false;	// removes the widget menu whenever the user clicks off any widgets...
					this._currentSprite = null;
					this._isWidgetMenuUsed = false;
				}
			}
			...
			this._isWidgetMenuCalledFor = false;
