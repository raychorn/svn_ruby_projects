...
package sho.ui
{
	...
	public class CompletionInput extends mx.controls.TextInput
	{
		...
		[Event(name="autoCompletionDone", type="sho.events.AutoCompletionDoneEvent")]
		[Event(name="autoCompletionReset", type="sho.events.AutoCompletionResetEvent")]
			...
			do
			{
				...
				else if (sortedCandidates != null)
				{
					...
					if (_completions.length > 0)
					{
						...
						if (prefix != this.text)
						{
							range2 = ArrayUtil.binarySearchForRange(_completions, this.text, searchFunction );
							if (range2 != null)
							{
...
//			trace("completion took " + diff + "ms");
		...
		public function displayCompletions(indexToSelect: int = 0) : void
		{
			...
			if (_completions != null && _completions.length > 0)
			{
				...
				this.dispatchEvent(new AutoCompletionResetEvent(AutoCompletionResetEvent.TYPE_AUTO_COMPLETION_RESET));
			}
			else
			{
				hideCompletions();
		...
		public function acceptCompletion() : void
		{
			if (completionWidget != null)
			{
				if (completionWidget.selectedIndex >= 0)
				{
					...
					{
						if (_labelField) {
							newText = _completions[index][_labelField];
						} else {
					...
					this.dispatchEvent(new AutoCompletionDoneEvent(AutoCompletionDoneEvent.TYPE_AUTO_COMPLETION_DONE, _selectedIndex));
				}
		...
		//----------------------------------
		private var _typedTextChangeEventDelay:int = 0;
		public function set typedTextChangeEventDelay(value:int):void {
			_typedTextChangeEventDelay = value;
			timer = new Timer(_typedTextChangeEventDelay, 1);
			timer.addEventListener(TimerEvent.TIMER, offerCompletions);
		}
		public function get typedTextChangeEventDelay():int {
			return _typedTextChangeEventDelay;
		}
		...
		private function _onCreationComplete(event: Event) : void
		{
			...
			if (_labelField) {
				completionWidget.labelField = _labelField;
			} else {
