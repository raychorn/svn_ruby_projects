	...
	public class RemoteWebOrbObject extends RemoteObject {
		...
		private function genericFaultHandler(event:FaultEvent):void {
			...
			if (this.onFaultHandler is Function) {
				try { this.onFaultHandler(event); } catch (err:Error) { AlertPopUp.error(err.toString(), "RemoteWebOrbObject::genericFaultHandler failed in onFaultHandler handler.") }
			} else {
		...
		private function genericResultHandler(event:ResultEvent):void {
			...
			try {
	   			if (event.result.status < 0) {
					...
					try {
						var vo:* = event.result;
						if (this.onResultHandler is Function) {
							try { this.onResultHandler(event, vo); } catch (err:Error) { AlertPopUp.error(err.toString(), "RemoteWebOrbObject::genericResultHandler failed in onResultHandler handler.") }
						} 
					} catch (err:Error) {
		   				sError = ObjectUtil.toString(event.result); //  + "\n" + sError
