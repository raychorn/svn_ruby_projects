	...
	public class SaveDashboardCommand implements ICommand, IResponder {
		...
		public function result(data:Object):void {
			if (this.resultHandler is Function) {
				try { this.resultHandler(data.result); } catch (err:Error) { AlertPopUp.error(err.toString(), "SaveDashboardCommand failed in result handler.") }
			}
		}
		...
		public function fault(info:Object):void {
			...
			if (this.faultHandler is Function) {
				try { this.faultHandler(info); } catch (err:Error) { AlertPopUp.error(err.toString(), "SaveDashboardCommand failed in error handler.") }
			}
		}
	}
}